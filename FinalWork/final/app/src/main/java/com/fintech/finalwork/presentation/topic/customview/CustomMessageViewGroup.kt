...
class CustomMessageViewGroup @JvmOverloads constructor(
    ...
    private fun updateView() {
        findViewById<TextView>(R.id.userName).text = message.sender_name
        findViewById<TextView>(R.id.messageText).text = message.content
        if (isHeader) {
    ...
    fun updateReactions(reactions: MutableMap<String, MutableSet<Long>>) {
        val flex = this.findViewById<FlexibleLayout>(R.id.flex)
        flex.removeAllViews()
        reactions.forEach {
            if (it.value.size != 0) {
                val emoji = it.key.toUpperCase(Locale.ROOT)
                ...
                flex.getChildAt(flex.childCount - 1).setOnClickListener {
                    listenerEmoji.onEmojiFlexClick(
                        position,
                        Emoji.valueOf(emoji.toUpperCase(Locale.ROOT))
                    ) //hardcoded id
                }
            }
        }
