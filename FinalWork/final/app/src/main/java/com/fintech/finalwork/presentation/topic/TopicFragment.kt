...
import com.fintech.finalwork.databinding.FragmentTopicBinding
...
import com.fintech.finalwork.presentation.topic.message.MessageListAdapter
import com.fintech.finalwork.presentation.topic.message.MessageViewModel
import com.fintech.finalwork.presentation.topic.reaction.BottomSheetDialogListener
import com.fintech.finalwork.presentation.topic.reaction.EmojiBottomSheet
import com.fintech.finalwork.presentation.topic.reaction.OnEmojiFlexClickListener
...
class TopicFragment : ElmFragment<Event, Effect, State>(), OnEmojiFlexClickListener,
    ...
    private fun initUi() {
        requireActivity().window.statusBarColor = resources.getColor(R.color.teal_700)
        binding.toolbar.title = requireArguments().getString(ARG_PARENT_NAME)
            ...
            override fun onScrollStateChanged(recyclerView: RecyclerView, newState: Int) {
                ...
                if (firstVisibleElement == 0) {
                    viewModelMessage.searchMessages(
                        SearchQuery(
                            numBefore = ((store.currentState.messages.size
    ...
    override fun onEmojiFlexClick(messagePosition: Int, emoji: Emoji) {
        ...
        messageAdapter.notifyItemChanged(messagePosition, MessageListAdapter.Payloads.EMOJI_CHANGE)
    }
    ...
    companion object {
        ...
        @JvmStatic
        fun newInstance(parent: Long, parentName: String, subject: String?) =
            TopicFragment().apply {
    ...
    override val initEvent: Event
        get() = Event.Ui.LoadMessages(
            SearchQuery(
                numBefore = StoreFactoryMessages.PAGING_COUNT,
                numAfter = 0,
                stream = requireArguments().getLong(ARG_PARENT),
                subject = requireArguments().getString(ARG_SUBJECT).toString()
            )
